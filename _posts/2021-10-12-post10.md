...
SQL에도 수많은 연산자가 있다. 산술연산자, 비교연산자.. 등등 대충 다른 프로그래밍 언어도 그랬듯이 <br>
연산자가 더 거기서 거기지 라는 마음으로 넘어가려는데, 좀 생소한 연산자가 몇개 보이길래 정리하고자 한다.

1. (NOT) LIKE
    - 유사 문자열 검색 (python startswith 와 유사)

    - A로 시작하는 모든 유저 네임을 출력하는 예시
        <pre><code>SELECT * FROM user_table WHERE user_name LIKE 'A%';</code></pre>
        - '%': 길이를 알 수 없는 문자 (0 이상)
        - '_': 문자열 한개
...
2. (NOT) IN:
    ...
    - 사는 곳이 '경주', '부산' 인 유저 네임을 출력하는 예시
        <pre><code>SELECT * FROM user_table WHERE user_location IN '경주' OR '부산';</code></pre>

3. (NOT) BETWEEN a AND b
    - 주로 범위/기간을 제한하여 조건을 걸 때 사용

    - 나이가 20 ~ 30 대인 유저 네임을 출력하는 예시
        <pre><code>SELECT * FROM user_table WHERE user_age BETWEEN 20 AND 30;</code></pre>
...
- INSERT 문
    ...
    - 기본 구조는 아래와 같다. ([자세한 문법 참고](https://dev.mysql.com/doc/refman/8.0/en/insert.html))
        ...
        <pre><code class="language-sql">INSERT INTO &lt;table_name&gt;(&lt;column1_name&gt;, &lt;column2_name&gt;, ...)<br>VALUES (&lt;column1_data&gt;, &lt;column2_data&gt;)</code></pre>
...
- UPDATE 문
    ...
    - 기본 구조는 아래와 같다. ([자세한 문법 참고](https://dev.mysql.com/doc/refman/8.0/en/update.html))
        ...
        <pre><code class="language-sql">UPDATE &lt;table_name&gt;<br>SET &lt;column_name&gt;=&lt;column_data&gt;, ...<br>WHERE &lt;condition&gt; </code></pre>
...
- DELETE 문
    ...
    - 기본 구조는 아래와 같다. ([자세한 문법 참고](https://dev.mysql.com/doc/refman/8.0/en/delete.html))
        ...
        <pre><code class="language-sql">DELETE FROM &lt;table_name&gt;<br>WHERE &lt;condition&gt;</code></pre>
...
SELECT &lt;column_name&gt;,&lt;column_name&gt;,... AS &lt;alias&gt; FROM &lt;table_name&gt;   -- 테이블의 일부 칼럼을 가져오면서, 테이블에 대한 별칭을 지정한다.</code></pre>
        ...
        - 위 기본 구조에서 FROM 절 뒤로 'WHERE', 'JOIN', 등등의 여러 추가적인 절이 존재하는데, 개인적으로 중요하다라고 생각된 몇 가지만 아래에 정리 한다.
            ...
            - JOIN 절
                ...
                - JOIN 관계도 ([출처](https://dsin.wordpress.com/2013/03/16/sql-join-cheat-sheet/))
                    <img border="1" src="{{ site.baseurl }}/public/images/post/post10/sql_join_cheat_sheet.png">
...
쿼리 안에 있는 쿼리, WHERE/FROM/SELECT 절 안에 들어가는 또 다른 쿼리를 통칭하여 Sub-Query 라고 부른다.<br>
~~(밑에 설명과 예시를 기록했는데... 아직 SQL에 익숙하지 못하다보니 쿼리문이 복잡한거 같다..)~~

- Sub-Query ([출처](http://www.complexsql.com/subqueries-correlated-subquery/))
    <img border="1" src="{{ site.baseurl }}/public/images/post/post10/sql_sub_query.png">


1. WHERE 절 Sub-Query
    - WHERE 절 안에 포함되어 있으며, Nasted subqueries (중첩 서브 쿼리) 라고도 불린다.

    - 'OS' 과목을 수강중인 학생 목록을 출력하는 예시 

        <pre><code class="language-sql">SELECT *<br>FROM AS st<br>WHERE st.id IN (<br>    SELECT c.student_id<br>    FROM class_registration_list AS c<br>    WHERE c.subject_id=(<br>        SELECT sb.id<br>        FROM subject_list AS sb<br>        WHERE sb.subject_name='OS'<br>    )<br>);</code></pre>


2. FROM 절 Sub-Query
    - FROM 절 안에 포함되어 있으며, Inline Views (인라인뷰) 라고도 불린다.

    - 각 학생들이 수강신청한 과목의 갯수를 가져오는 예시

        <pre><code>SELECT st.name AS '이름', class_list.registor_total AS '수강 신청 과목 개수'<br>FROM student_list AS st, (<br>    SELECT c.student_id, COUNT(c.subject_id) AS registor_total<br>    FROM class_registration_list AS c<br>    GROUP BY c.student_id<br>) class_list<br>WHERE class_list.student_id = st.id;</code></pre>
        - 'class_registration_list' 테이블에서 각 학생들이 신청한 수강 과목의 개수를 계산하여, 'registor_total' 로 저장
        - 이후 Main-Query 에서 해당 값을 사용하여 출력

3. SELECT 절 Sub-Query
    - SELECT 절 안에 포함되어 있으며, Scalar subqueries (스칼라서브쿼리) 라고도 불린다.
    - 반드시 단일 값을 리턴해야 사용 가능하다. (때문에 SUM, COUNT, MIN, MAX  같은 함수가 많이 사용 된다.)

    - 각 과목을 수강신청한 학생수의 평균 값
		<pre><code>SELECT sb.subject_name, (<br>    SELECT COUNT(c.student_id)<br>    FROM class_registration_list AS c<br>    WHERE c.subject_id = sb.id<br>) AS subject_avg<br>FROM subject_list AS sb<br>ORDER BY subject_avg DESC;</code></pre>
